...
public:
  ...
  VTK_LEGACY(VTKCOMMONCORE_EXPORT void CopyEntry(vtkInformation* from, vtkInformationExecutivePortKey* key, int deep=0));

  // Description:
  // Append an InformationKey-vector-valued entry.
  // @deprecated Replaced by vtkInformationExecutivePortKey::Append() as of VTK 5.2.
  VTK_LEGACY(VTKCOMMONCORE_EXPORT void Append(
  ...
  VTK_LEGACY(VTKCOMMONCORE_EXPORT void AppendUnique(
      vtkInformationKeyVectorKey* key,
      vtkInformationExecutivePortKey* value));
  ...
  VTK_LEGACY(VTKCOMMONCORE_EXPORT
